describe('Navigation', () => {
    it('should do an mvp scenario of the test-application', () => {
        ...
        cy.get('[data-testid=select-realtors]').contains('option');
        cy.get('[data-testid=select-realtors]').select('101');
        cy.url().should('include', '/realtors/101')
        cy.get('[data-testid=message-list-container]').scrollTo('bottom');
        cy.url().should('include', '/realtors/101?page=2')
        cy.get('[data-testid=message-list-container] >li:first-child').click();
    ...
    it('should navigate between agency by url', () => {
        ...
        cy.get('[data-testid=message-list-container]').contains('li');
    ...
    it('should navigate between agency by selecting agency', () => {
        ...
        cy.url().should('include', '/realtors/101')
        cy.get('[data-testid=message-list-container]').contains('li');
        cy.get('[data-testid=select-realtors]').select('102');
    ...
    it('should navigate load more message when scrolling to bottom on Desktop', () => {
        cy.visit('http://localhost:3000/realtors/101');
        cy.get('[data-testid=message-list-container]').children().should('have.length', 10);
        cy.get('[data-testid=message-list-container]').scrollTo('bottom');
        cy.url().should('include', '/realtors/101?page=2')
        cy.get('[data-testid=message-list-container]').children().should('have.length', 20);
        cy.get('[data-testid=message-list-container]').scrollTo('bottom');
        cy.url().should('include', '/realtors/101?page=3')
        cy.get('[data-testid=message-list-container]').children().should('have.length', 30);
        cy.get('[data-testid=message-list-container]').scrollTo('bottom');
        cy.url().should('include', '/realtors/101?page=4')
        cy.get('[data-testid=message-list-container]').children().should('have.length', 40);
        cy.get('[data-testid=message-list-container]').scrollTo('bottom');
        cy.url().should('include', '/realtors/101?page=5')
        cy.get('[data-testid=message-list-container]').children().should('have.length', 50);
    })
    ...
    it('onClick unread message should decrement the unread message count', () => {
        ...
        cy.get('[data-testid=message-list-container]').scrollTo('bottom');
        cy.wait(1000)
        cy.get('[data-testid=unread-message]').invoke('text')
            .then((val1) => {
            cy.get('[data-testid=message-list-container] > li[data-testid-read="false"]:first').click();
            cy.get('[data-testid=unread-message]').invoke('text')
               .should((val2) => {
